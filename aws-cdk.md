...
pip install -r entsrequirements.txt       # cdk init generates this file
...
the file simply doesn't exist yet. It is generated by `cdk init`, not by `venv`. Creating a
virtual environment gives you an empty environment — it does not know anything about CDK.

Which fix applies depends on whether your directory is empty:

| Situation                                           | What to do                                                                 |
| --------------------------------------------------- | -------------------------------------------------------------------------- |
| Empty directory                                     | `cdk init app --language python` — generates everything                 |
| Directory already has files (README, docs,`.git`) | `cdk init` **refuses to run**. Write `requirements.txt` yourself |

The second case is common: you create a repo on GitHub with a README, clone it, and only then
reach for CDK. `cdk init` bails out with *"cdk init cannot be run in a non-empty directory"*.

Write the two files by hand — that is all `cdk init` would have put in them:
...
Verify the library imports:

```bash
python -c "import aws_cdk; print(aws_cdk.__version__)"
```

If you want the rest of the scaffolding (`app.py`, `cdk.json`, the stack package), generate it
in a throwaway directory and copy it across:

```bash
mkdir /tmp/cdk-scaffold && cd /tmp/cdk-scaffold
cdk init app --language python
cd -
cp -r /tmp/cdk-scaffold/{app.py,cdk.json,source.bat} .
cp -r /tmp/cdk-scaffold/cdk_scaffold ./my_app        # the stack package
rm -rf /tmp/cdk-scaffold
```

Then edit `cdk.json` so `"app"` points at your `app.py`, and rename the package import inside
`app.py` to match the folder you copied it to.

> `cdk init` also runs `git init`. In an existing repo that is harmless — it detects the
> existing `.git` and skips — but it is another reason the throwaway-directory route is
> cleaner than fighting the emptiness check.
...
| `Could not open requirements file: ... 'requirements.txt'`                                      | File not generated —`cdk init` was never run in this folder | Write it by hand (see[Could not open requirements file](#could-not-open-requirements-file))                  |
| `cdk init cannot be run in a non-empty directory`                                               | Directory already contains files such as a README              | Create`requirements.txt` manually, or scaffold in a temp dir and copy across                              |
